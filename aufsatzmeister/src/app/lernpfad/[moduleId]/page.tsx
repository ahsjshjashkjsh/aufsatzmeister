...
export default function ModulePage({ params }: Props) {
  ...
  async function handleComplete(quizScore: number) {
    ...
    await supabase.from('module_progress').upsert({
      user_id: user.id, module_id: moduleId, completed: true,
      quiz_score: quizScore, completed_at: new Date().toISOString(),
    })
  }
          ...
          {completed ? (
            ...
            <div>
              <div className="flex items-center gap-3 mb-6">
                <Link href="/lernpfad">
                  <button className="w-9 h-9 bg-white rounded-xl border border-stone-200 flex items-center justify-center hover:bg-stone-50 transition-colors shadow-sm">
